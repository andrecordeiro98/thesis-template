...
\begin{itemize}
\item \texttt{/01-Cover-Page} -- A cover according to \textit{IST} regulations. The names of the author, supervisors, and committee members must be added, as well as the name of the degree. You can also choose a cover image. Choose one of the following files:
\begin{itemize}
	\item \texttt{thesis-front-cover-msc-en}: My attempt at a MSc compliant cover (unsure of the regulations).
	\item \texttt{thesis-front-cover-phd-double-degree}: For your draft (used during the defense). Includes both an optional image and the logo of an external university, in case of a double degree.
	\item \texttt{thesis-front-cover-phd-double-degree-definitive}: One of the definitive covers. Includes the classification awarded by the jury. Includes an optional image.
	\item \texttt{thesis-front-cover-phd-double-degree-definitive-juryname}: The other definitive cover. Includes the classification awarded by the jury. Includes the name of the jury committee members and funding information, but no optional image.
\end{itemize}
