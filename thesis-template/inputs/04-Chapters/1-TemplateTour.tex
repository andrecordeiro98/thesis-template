...
A more recent addition has overhauled the cover pages, to make them compliant with the templates provided in \href{https://posgraduacao.tecnico.ulisboa.pt/programas-doutorais/provas-academicas/}{\textit{Área de Pós-Graduação}}. Nonetheless, clarifications are needed regarding type size (currently 10pt) and how to use the covers.
...
\begin{itemize}
	\item \verb|\usepackage[labelfont=bf,font=small]{caption}| is used to set ``\textbf{Figure X.Y:}'' to boldface. Same is done for subcaptions.
	...
	%\item \texttt{\textbackslash DeclareMathSizes{<tfs>}{<ts>}{<ss>}{<sss>}} is called to set the size of math mode, both in equations and inline, such as $\sqrt{x\upp{2} + 1}$, $f\sub{p/h}(x, Q\upp{2})$, or $\tau\ind{-1}{\sf form} \propto z \,\theta\upp{2}$. The arguments are respectively the size of surrounding text, the size for math mode, the size of subscripts, and the size of subsubscripts. It must be called twice, with \verb+<tfs>+ larger and smaller than the font size used in the document.
	...
	\item \verb|\captionsetup{width=.85\textbackslash textwidth}| is called to define the width of captions.
	\item The \texttt{openright} class option makes sure that the chapters start on odd-numbered pages. The \texttt{emptypage} package ensures that the blank pages are not numbered. Some blank pages had to be manually included to make sure the first chapter starts on the right hand side of the spread (on the front page of each leaf). To check how this will look when printed, select \textbf{``two page view (even spreads)''} in your pdf viewer.
\end{itemize}
...
\begin{itemize}
\item \texttt{/01-Cover-Page} -- Covers according to \textit{IST} regulations. The names of the author, supervisors, and committee members must be added, as well as the name of the degree. You can also choose a cover image. The commands inside \texttt{thesis-preamble.tex}, section \texttt{COVER PAGE TOOLS} must be edited with the names of the author, supervisors, the thesis title, etc. Available files:
\begin{itemize}
	\item \texttt{thesis-front-cover-msc}: For MSc dissertations. Should comply with regulations, including all of the type sizes. Includes space for an (optional) image.
	%
	\item \texttt{thesis-front-cover-draft}: For your draft (used during the defense). Includes both an optional image and the logo of an external university, in case of a double degree.
	%
	\item \texttt{thesis-front-cover-definitive-1}: Page 1 of the definitive covers. Includes the classification awarded by the jury. Includes an optional image.
	%
	\item \texttt{thesis-front-cover-definitive-2}: Page 2 of the definitive covers. Includes the classification awarded by the jury. Includes the names of the jury committee members and funding information, but no optional image.
\end{itemize}


%When announcing the name of the degree, user must choose between \verb|\coverThesisMSc| and \verb|\coverThesisPhD|.
